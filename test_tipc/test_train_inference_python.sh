...
input_dir_key=$(func_parser_key "${lines[30]}")
input_img_dir=$(func_parser_value "${lines[30]}")
output_dir_key=$(func_parser_key "${lines[31]}")
output_img_dir=$(func_parser_value "${lines[31]}")
    ...
    for use_gpu in ${use_gpu_list[*]}; do
        ...
        if [ ${use_gpu} = "False" ] || [ ${use_gpu} = "cpu" ]; then
            for batch_size in ${batch_size_list[*]}; do
                ...
                set_input_data=$(func_set_params "${input_dir_key}" "${_input_dir}")
                set_output_data=$(func_set_params "${output_dir_key}" "${_output_dir}")
                set_batchsize=$(func_set_params "${batch_size_key}" "${batch_size}")
                set_model_dir=$(func_set_params "${infer_model_key}" "${_model_dir}")
                command="${_python} ${_script} ${use_gpu_key}=${use_gpu} ${set_model_dir} ${set_batchsize} ${set_input_data} ${set_output_data} > ${_save_log_path} 2>&1 "
                eval $command
    ...
    for gpu in ${gpu_list[*]}; do
        ...
        for trainer in ${trainer_list[*]}; do
            ...
            if [ ${#gpu} -le 2 ];then  # train with single gpu
                ...
                cmd="${python} -m paddle.distributed.launch --gpus=${gpu} ${run_train} }"
            else     # train with multi-machine
                cmd="${python} -m paddle.distributed.launch --ips=${ips} --gpus=${gpu} ${run_train} "
            fi
            # run train
            eval $cmd
            ...
            if [ ${run_export} != "null" ]; then
                ...
                func_inference "${python}" "${inference_py}" "${infer_model_dir}" "${LOG_PATH}" "${input_img_dir}" "${output_img_dir}"
